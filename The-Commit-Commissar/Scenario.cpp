...
bool Scenario::execute()
{
    ...
    for (uint32_t i = 0; i < instructionCount; i++)
    {
        ...
        std::string setEnvironmentVariablesCmd;
        if (instructionObject->hasProperty("environmentVariables"))
        {
            uint32_t variablesCount = instructionObject->getArraySize("environmentVariables");
            for (uint32_t variableIdx = 0; variableIdx < variablesCount; variableIdx++)
            {
                Wolf::JSONReader::JSONObjectInterface* variableObject = instructionObject->getArrayObjectItem("environmentVariables", variableIdx);
                std::string variableName = variableObject->getPropertyString("name");
                std::string variableValue = variableObject->getPropertyString("value");

                setEnvironmentVariablesCmd += variableName + "=\"" + variableValue + "\" && ";
            }
        }
        ...
        if (instructionCommand == "executeWithoutResult" || instructionCommand == "executeWithResult")
        {
            ...
            std::string commandStr = absoluteExePath.string();

            std::error_code ec;
            bool commandIsAFile = std::filesystem::is_regular_file(absoluteExePath, ec);
            if (!commandIsAFile)
            {
                commandStr = instructionValue;
            }

            std::string cmd = setEnvironmentVariablesCmd;
            ...
            if (instructionObject->hasProperty("workingDirectory"))
            {
                cmd += "(cd \"" + workingDirectoryStr + "\" && \"" + commandStr + "\"";
            }
            else if (commandIsAFile)
            {
                cmd += "\"" + commandStr + "\"";
            }
            else
            ...
            if (commandIsAFile && instructionObject->hasProperty("commandLineArguments"))
            {
                cmd += " " + instructionObject->getPropertyString("commandLineArguments");
            }
        ...
        else if (instructionCommand == "buildCMake")
        {
            ...
            std::string generateCommand = setEnvironmentVariablesCmd + goToFolderCmd + "&& cmake -S . -B build -G \"" + compiler + "\"" + " " + buildOptions;
            ...
            std::string buildCommand = setEnvironmentVariablesCmd + goToFolderCmd + "&& cmake --build build";
...
#elif __linux__
            std::string instructionValue = "xterm -title \"" + windowTitle + "\" -hold -e \"" + instructionObject->getPropertyString("value") + "\" &";
#endif
...
#elif __linux__
            std::string command = "adb -s " + device + " install \"" + fullPathToAPK + "\"";
#endif
...
#elif __linux__
            std::string command = "xterm -title \"" + windowTitle + "\" -hold -e \"adb -s " + device + " shell am start -n " + appName + "\" &";
#endif

            system(command.c_str());
        }
        else if (instructionCommand == "androidScreenshot")
        {
            std::string device = instructionObject->getPropertyString("device");
            std::string filename = instructionObject->getPropertyString("filename");

            std::string command = "adb -s " + device + " exec-out screencap -p > \"" + m_cloneFolder + "/" + filename + "\"";
            executeCommandWithLogs(command);

            filesCreated.push_back(m_cloneFolder + "/" + filename);
        }
...
#elif __linux__
        std::string killCmd = "pkill -9 -f \"" + windowTitle + "\"";
#endif
    ...
    if (hadAnError)
    {
        Wolf::Debug::sendInfo("Scenario failed: " + errorMessage);
    }
...
std::string Scenario::computeAbsoluteFullPath(const std::string& pathStr) const
{
    std::filesystem::path path(pathStr);
    if (path.is_absolute())
    {
...
std::string Scenario::computeStatusString(Status status)
{
    switch (status)
    {
        case Status::SUCCESS:
