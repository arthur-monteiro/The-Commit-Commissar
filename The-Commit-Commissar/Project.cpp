...
std::string extractRepoName(std::string url)
{
    ...
    size_t gitExt = name.find(".git");
    if (gitExt != std::string::npos)
    {
        name.erase(gitExt);
    }
...
Project::Project(const std::string& name, const std::string& repoURL, const std::string& username, const std::string& sourceBranchName, const std::string& targetBranchName, Wolf::JSONReader::JSONObjectInterface* scenarioObject,
    const std::function<void()>& reloadOutputJSONCallback)
: m_name(name), m_repoURL(repoURL), m_username(username), m_sourceBranchName(sourceBranchName), m_targetBranchName(targetBranchName)
{
...
Project::MergeResult Project::merge()
{
    ...
    std::string setOriginCmd = "cd " + targetCloneFolder + " && git remote set-url origin git@github.com:" + m_username + "/" + extractRepoName(m_repoURL);
    if (executeCommandWithLogs(setOriginCmd) != 0)
    {
        m_isInError = true;
        return MergeResult::SET_ORIGIN_FAILURE;
    }
